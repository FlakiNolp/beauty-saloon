    ...
    async def _get_days_kb(self, year: int, month: int):
        ...
        def format_day_string():
            ...
            if self.min_date and date_to_check < self.min_date:
                ...
                return superscript(str(day))
            return (f'✅' if date_to_check.date() in self.exist_dates else str(day)) if self.exist_dates is not None else str(day)
        ...
        callback_data = [i.day for i in self.exist_dates] if self.exist_dates is not None else None
        for week in month_calendar:
            ...
            for day in week:
                ...
                days_row.append(InlineKeyboardButton(
                    text=highlight_day(),
                    callback_data=((DialogCalendarCallback(act=DialogCalAct.day, year=year, month=month, day=day).pack() if day in callback_data else self.ignore_callback) if callback_data is not None else DialogCalendarCallback(act=DialogCalAct.day, year=year, month=month, day=day).pack())
                ))
            kb.append(days_row)
